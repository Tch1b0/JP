...
import showdown from "../../node_modules/showdown/dist/showdown"
...
export default {
    ...
    methods: {
        getData() {
            axios.get(`https://api.johannespour.de/post/${this.$route.params["name"]}`)
            .then((response) => {
                this.title = response.data["title"];
                if (this.title === undefined) { window.location = "/"; }
                ...
                this.longDescription = this.markdownToHtml(response.data["long-description"]);
            })
            .catch(() => {
                window.location = "/";
            });
